...
Sea \(f(x) = x e^x\). Aproxime \(f^\prime (2)\) mediante la
\emph{fórmula centrada de tres puntos} \[
f^\prime(x) = \frac{f(x+h) - f(x-h)}{2h} + \mathcal{O}(h^2)
\] para distintos valores de \(h\) y calcule el incremento óptimo
\(h_o\) teniendo en cuenta los errores de truncamiento y redondeo.
Usando la diferencia entre \(f'(2)\) (calculado en Julia usando las
funciones matemáticas incorporadas) y el valor de la aproximación
mencionada, grafique en escala \emph{log-log} el error versus \(h\) para
\(h = 10^{-k}\) con \(k\) entero.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Implemente el cálculo aproximado de las derivadas de \(f(x)=\cos x\) y
  \(g(x)=e^x\) en \(x=0.1, 1, 100\) utilizando las fórmulas \emph{i)
  hacia adelante de 2 puntos}, \emph{ii) centrada de 3 puntos} y
  \emph{iii) centrada de 5 puntos}.
\item
  Haga un gráfico \emph{log-log} del error relativo \(E=\) vs \(h\) para
  \(h\in [\epsilon_m(x),0.1]\) (donde \(\epsilon_m(x)\) es la precisión de la
  máquina al valor $x$).
\item
...
\item
  ...
  puntos. Re-obtenga la fórmula dada en el teórico para
