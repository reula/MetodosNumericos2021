...
\item
  ...
  \(g(x)=e^x\) utilizando las fórmulas \emph{i) hacia adelante de 2 puntos}, 
  \emph{ii) centrada de 3 puntos} y \emph{iii) centrada de 5 puntos}.
\item
  Haga un gráfico \emph{log-log} del error relativo \(E\) vs \(h\) para
  \(h\in [\epsilon_m(x),0.1]\) y $x=0.1, 1, 100$. Aquí $\epsilon_m(x)$ es la 
  precisión de la máquina al valor $x$.
\item
